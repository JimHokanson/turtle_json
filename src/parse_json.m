function obj = parse_json(str,j,numeric_data)

in.collapse_array = true;

if j(1,1) == 1
    obj = parse_object(str,j,1,numeric_data);
elseif j(1,1) == 2
    obj = parse_array(str,j,1,numeric_data);
end
...
function obj = parse_object(str,j,index,numeric_data)
...
for iItem = 1:j(4,index)
    ...
    switch(j(1,cur_value_I))
        case 1
            value = parse_object(str,j,cur_value_I,numeric_data);
        case 2
            value = parse_array(str,j,cur_value_I,numeric_data);
        case 3
...
function array = parse_array(str,j,index,numeric_data)
n_items = j(4,index);
if n_items == 0
